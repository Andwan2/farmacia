...
import 'package:farmacia_desktop/models/cliente.dart';
  ...
  Future<void> cargarClientes() async {
    ...
    try {
      ...
      for (var cliente in _clientes) {
        print('   • ${cliente.nombreCliente} (tel: ${cliente.numeroTelefono ?? "N/A"})');
      }
  ...
  Future<String?> guardarVenta() async {
    ...
    try {
      ...
      for (var producto in _productos) {
        ...
        print('   • ${producto.presentacion}: ${producto.cantidad} requeridos, $count disponibles');
      ...
      final ventaResponse = await Supabase.instance.client
          .from('venta')
          .insert({
            'fecha': _fecha.toIso8601String().split('T')[0], // Solo la fecha YYYY-MM-DD
            'total': total,
      ...
      for (var producto in _productos) {
        print('   📦 Procesando: ${producto.presentacion} x${producto.cantidad}');
        ...
        print('      ✅ ${idsProductos.length} productos marcados como Vendidos');
        
        // Insertar en producto_en_venta
        final productosEnVenta = idsProductos.map((idProducto) => {
